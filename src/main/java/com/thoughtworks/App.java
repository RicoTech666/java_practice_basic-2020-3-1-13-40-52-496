...
public class App {
    ...
    public static boolean hasMilanTrader(List<Transaction> transactions) {
        return transactions.stream().anyMatch(transaction -> "Milan".equals(transaction.getTrader().getCity()));
    }

    public static List<Integer> getCambridgeTransactionsValue(List<Transaction> transactions) {
        return transactions.stream().filter(transaction -> "Cambridge" == transaction.getTrader().getCity())
    ...
    public static Transaction getMinTransaction(List<Transaction> transactions) {
        return transactions.stream().min(Comparator.comparing(Transaction::getValue)).orElseThrow(NoSuchElementException::new);
    }
}
